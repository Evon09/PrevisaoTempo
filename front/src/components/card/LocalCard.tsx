...
interface LocalCardProps {
  local: string; // Adicione uma propriedade local do tipo string
}
...
export function LocalCard({ local }: LocalCardProps) {
    ...
    const fetchWeatherData = async (latitude: number, longitude: number) => {
      try {
        const response = await fetch(
          `https://api.openweathermap.org/data/2.5/weather?q=${local}&units=metric&appid=${API_KEY}&lang=pt_br`
        );
  ...
  return (
    ...
    >
      <Flex top="10" left="10" position="absolute" zIndex="100">
        {auth.email ? <AddFavorite cidade={local}></AddFavorite> : <></>}
      </Flex>
      <Image src=""></Image>
      <Flex
        direction="column"
